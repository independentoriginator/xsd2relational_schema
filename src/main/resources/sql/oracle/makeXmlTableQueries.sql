select
    ...
    || LF || '		'
    || case
        when target_column.namespaces is not null then
            'xmlnamespaces('
            || LF || '			' || target_column.namespaces
            || LF || '		)'
            || LF || '		, '
    end
       ...
       dbms_xmlgen.convert(
           xmlagg(
               xmlelement(
                   ...
                   || ' path ''' || target_column.absolute_path || '''' -- relative paths are not supported in Oracle due to context node limitations
                   , LF || '			, '
               ).extract('//text()')
               order by
